...
import { initFirebaseAdmin, getFirestoreInstance } from "./firebaseAdmin";
 ...
 * - Authoritative cloud storage: Firestore collections `current_affairs` and `daily_mcqs`
 * - Resilient single-instance cache: `models/current_affairs_store.json` (for local development, offline mode, and fast boot)
 */

// On Vercel (and other read-only serverless filesystems) only /tmp is writable.
// Firestore remains the authoritative store either way; this path is purely a
// local/offline resilience cache, so it's safe to relocate per-runtime.
const DATA_STORE_ROOT =
  process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME
    ? path.join(os.tmpdir(), "bolt-models")
    : path.join(process.cwd(), "models");
const DATA_STORE_PATH = path.join(DATA_STORE_ROOT, "current_affairs_store.json");
...
// In-memory cache backed by Firestore + local disk
...
function getFirestoreDb() {
  try {
    ...
    // Offline, mock, or test environments
  }
  return null;
}
...
export function loadCurrentAffairsFromDisk(): NewsArticle[] {
  try {
    ...
    console.warn("Failed reading current_affairs_store.json cache:", e);
  }
  return cachedArticles;
}
...
export function saveCurrentAffairsToDisk(articles: NewsArticle[], mcqs?: PrelimsQuestion[]) {
  try {
    ...
    console.error("Failed saving current_affairs_store.json cache:", e);
  }
}

/**
 * Authoritative cloud persistence: writes articles and MCQs to Firestore collections
 * `current_affairs` and `daily_mcqs`, while keeping the local disk cache in sync.
 */
export async function saveCurrentAffairsToFirestore(
  articles: NewsArticle[],
  mcqs?: PrelimsQuestion[]
): Promise<void> {
  // Always update local disk cache first
  saveCurrentAffairsToDisk(articles, mcqs);

  const db = getFirestoreDb();
  if (!db) return;

  try {
    const targetMcqs = mcqs || cachedMcqs;
    const BATCH_SIZE = 400; // Under Firestore 500-operation limit

    // Save articles
    const articleDocs = articles.slice(0, 100);
    for (let i = 0; i < articleDocs.length; i += BATCH_SIZE) {
      const chunk = articleDocs.slice(i, i + BATCH_SIZE);
      const batch = db.batch();
      for (const art of chunk) {
        if (art && art.id) {
          const ref = db.collection("current_affairs").doc(String(art.id));
          batch.set(
            ref,
            {
              ...art,
    ...
    // Save MCQs
    const mcqDocs = targetMcqs.slice(0, 50);
    for (let i = 0; i < mcqDocs.length; i += BATCH_SIZE) {
      const chunk = mcqDocs.slice(i, i + BATCH_SIZE);
      const batch = db.batch();
      for (const mcq of chunk) {
        if (mcq && mcq.id) {
          const ref = db.collection("daily_mcqs").doc(String(mcq.id));
          batch.set(
            ref,
            {
              ...mcq,
    ...
    console.warn("Firestore current affairs sync notice:", err.message);
  }
}

/**
 * Authoritative cloud retrieval: loads articles and MCQs from Firestore collections,
 * falling back to local disk cache and automatically seeding Firestore if empty.
 */
export async function loadCurrentAffairsFromFirestore(): Promise<{
  articles: NewsArticle[];
  mcqs: PrelimsQuestion[];
}> {
  // Prime in-memory cache from disk first
  loadCurrentAffairsFromDisk();

  const db = getFirestoreDb();
  if (db) {
    try {
      const [articlesSnap, mcqsSnap] = await Promise.all([
        db.collection("current_affairs").limit(100).get(),
        db.collection("daily_mcqs").limit(50).get(),
      ]);

      if (!articlesSnap.empty) {
        const firestoreArticles: NewsArticle[] = [];
        articlesSnap.forEach((doc) => {
          firestoreArticles.push(doc.data() as NewsArticle);
        });

        const firestoreMcqs: PrelimsQuestion[] = [];
        if (!mcqsSnap.empty) {
          mcqsSnap.forEach((doc) => {
            firestoreMcqs.push(doc.data() as PrelimsQuestion);
          });
        }

        cachedArticles = firestoreArticles;
        if (firestoreMcqs.length > 0) {
          cachedMcqs = firestoreMcqs;
        }

        // Keep local disk updated as cache
        saveCurrentAffairsToDisk(cachedArticles, cachedMcqs);
        return { articles: cachedArticles, mcqs: cachedMcqs };
      } else {
        // If Firestore is empty, seed it from the pre-existing disk cache
        if (cachedArticles.length > 0 || cachedMcqs.length > 0) {
          saveCurrentAffairsToFirestore(cachedArticles, cachedMcqs).catch(() => {});
        }
      }
    } catch (e: any) {
      console.warn("Firestore current affairs fetch notice (falling back to disk cache):", e.message);
    }
  }
...
export async function executeNewsIngestionPipeline(): Promise<{
  ...
  const { articles: existingArticles } = await loadCurrentAffairsFromFirestore();
  const beforeCount = existingArticles.length;
  const merged = deduplicateArticles(collected, existingArticles);
...
export function generateDailyCurrentAffairsMCQs(articles: NewsArticle[], count: number = 5): PrelimsQuestion[] {
  ...
  cachedMcqs = mergedMcqs;
  saveCurrentAffairsToDisk(cachedArticles, cachedMcqs);
  // Authoritative cloud persistence
  saveCurrentAffairsToFirestore(cachedArticles, cachedMcqs).catch(() => {});
  return mcqs;
}
